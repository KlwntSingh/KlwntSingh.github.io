...
* #### 3-errors 
	* errors exists that can affect functionality 
* #### 4-warnings 
...
* #### 7-debug 
	* diagnostic issues for investigating issues 
* generally, log level should be notification or higher 
* information log level too generates lots of log 
* debug level to be used when actively debugging issues 
...
## local logging 
* #### logging on FGT meaning logging on internal hard disk in FGT 
	* logs are stored in sql database 
* local logging can impact performance 
* traffic logs and log archival are usually big 
* when using local hard disk for wan optimization then local logging cannot be enabled unless using two separate disks one for wan optimization and one for logging 
* #### for local logging, disk logging has to be enabled 
	* fortiview can use logs on disk for historical view but when no logs on disk then fortiview can only show real-time logs 
* logs older than 7 days are deleted by default 

## reserved disk space 
* 25% is reserved for system usage, quota overflow 
* 75% to store logs 
